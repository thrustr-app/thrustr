...
//
// The Windows and Linux window controls are adapted from Zed
// (https://github.com/zed-industries/zed), `platform_title_bar` crate,
// Copyright (C) Zed Industries, Inc., GPL-3.0-or-later.

use gpui::{
    AnyElement, App, Decorations, ElementId, FontWeight, Hsla, InteractiveElement, IntoElement,
    MouseButton, ParentElement, RenderOnce, Rgba, StatefulInteractiveElement, Styled, Window,
    WindowButton, WindowButtonLayout, WindowControlArea, div, prelude::FluentBuilder, px, rems,
    svg,
};
use theme::ThemeExt;

pub const TITLE_BAR_HEIGHT: gpui::Rems = gpui::Rems(2.);
const TRAFFIC_LIGHT_WIDTH: f32 = 72.;
...
impl RenderOnce for TitleBar {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let leading_pad = macos && !window.is_fullscreen() && left.is_none();
        ...
        div()
            ...
            // The window-control-area drag hitbox is a no-op on Linux, so move
            // the window explicitly when the empty title-bar region is grabbed.
            .when(platform == PlatformStyle::Linux, |this| {
...
/// Native Windows 11 caption-button width.
